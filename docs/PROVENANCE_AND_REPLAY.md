# Provenance: observed decisions, audit, and explicit replay

The integrated workflow calls these same facilities from `ResearchAssistant.run()`. With tracking enabled before
...
PyAutoStat provides optional local tracking and separate read-only consistency checks. These do not change execution calculations or interpretation rules. Base reports retain schema version 1; optional follow-up content uses additive schema version 2. An audit pass means only that implemented checks found no discrepancy in the inspected records and exports. It does not validate study design, raw data authenticity, causal claims, or preregistration.

Follow-up analyses add local events for `sensitivity_plan_created`, every scenario attempt and outcome, and
`meaningful_threshold_declared`. The plan is recorded before scenario execution within the call,
but its `planning_status` is only the caller's declaration. It is not external preregistration.
Unavailable and failed attempts are retained. Repeated threshold calls preserve previous and new
values rather than rewriting history.
...
`assistant.audit(report, result=None, sensitivity=None, practical_significance=None, exports=None)` reads the source snapshots retained by reports built through `assistant.report()`. A report reconstructed directly from a dictionary needs explicitly supplied original records for a complete audit; otherwise status is `incomplete`. The auditor rebuilds the expected report from those sources using the existing interpreter, without running a statistical test or sensitivity scenario. It compares the specification, recommendation, executed method, sample and group accounting, numerical values, effect and interval records, interpretation, warnings, limitations, structured sections, and table cells. For follow-up analyses it also compares scenario count/order/status, method, comparability, estimate/CI/sample/p-value fields, threshold, point/interval relation, and conclusion. This is an exact comparison of recorded canonical snapshots; no display rounding can conceal a changed p-value. Existing interpretation validation governs interval semantics, including valid percentile-bootstrap intervals outside the original estimate and partial interpretations with a missing effect component.
...
Base-only records remain reproducibility schema version 1. Calling
`reproducibility_record(result, sensitivity=..., practical_significance=...)` creates schema
version 2 and adds ordered scenario specifications, actual methods, outcomes, seeds, dataset
fingerprint metadata, sensitivity content reference, meaningful threshold, and practical-result
reference. It does not embed scenario DataFrames or rerun anything.

`reproduce(record, data=frame)` first checks a recorded fingerprint. A mismatch stops before execution by default. `allow_changed_data=True` permits a labelled changed-data rerun, never a same-data reproduction. Without a usable fingerprint, replay can compare numbers but returns `unavailable` for the same-data claim. It revalidates the specification, requires the same method to remain selected, then calls the existing execution engine only on explicit request. Valid Student t and one-way ANOVA adapter records cannot be silently routed through a different guided selector choice. The original result is never replaced. For schema version 2, replay still executes only the base analysis and warns that recorded follow-up analysis scenarios were not automatically replayed.
...
## Planning and presentation records
