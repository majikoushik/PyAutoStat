...
The Phase 1 `ResearchAssistant(df).profile()` facade through Phase 11 sensitivity and meaningful-threshold follow-up are available locally. The uncommitted Phase 12 working tree adds analysis plans, prospective study planning, explicit two-condition paired means, completeness checks, oriented presentation, safe LaTeX, and a session snapshot pending owner review. Reporting and auditing read recorded results; only an explicitly requested replay reruns the base test. The [development roadmap](DEVELOPMENT_ROADMAP.md) defines the phase sequence; this older table remains a high-level summary.
