...
| 3. Automatic test selection | `hypothesis_tests(test_type="auto", estimand=...)` selects supported independent-group methods. The `recommend_test()` path checks stated design and target; `analyze()` now executes ready runnable recommendations. | Add justified multi-group mean support and later interpretation integration. |
...
The Phase 1 `ResearchAssistant(df).profile()` facade, Phase 3 dataset intelligence, Phase 4 `prepare_question()` intake, Phase 5 `recommend_test()` design review, and Phase 6 `analyze()` execution are available locally. Question intake records unresolved research facts; recommendation checks method compatibility; execution runs only a freshly selected existing backend. The [development roadmap](DEVELOPMENT_ROADMAP.md) defines the phase sequence; this older table remains a high-level summary.
