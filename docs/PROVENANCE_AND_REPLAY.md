...
Phase 9 adds optional local tracking and separate read-only consistency checks. It does not change the Phase 6 calculations or Phase 7 rules. Base reports retain Phase 8 schema version 1; Phase 11 uses an additive schema version 2 only when its optional content is supplied. An audit pass means only that implemented checks found no discrepancy in the inspected records and exports. It does not validate study design, raw data authenticity, causal claims, or preregistration.

Phase 11 adds local events for `sensitivity_plan_created`, every scenario attempt and outcome, and
...
`assistant.audit(report, result=None, sensitivity=None, practical_significance=None, exports=None)` reads the source snapshots retained by reports built through `assistant.report()`. A report reconstructed directly from a dictionary needs explicitly supplied original records for a complete audit; otherwise status is `incomplete`. The auditor rebuilds the expected report from those sources using the existing Phase 7 interpreter, without running a statistical test or sensitivity scenario. It compares the specification, recommendation, executed method, sample and group accounting, numerical values, effect and interval records, interpretation, warnings, limitations, structured sections, and table cells. For Phase 11 it also compares scenario count/order/status, method, comparability, estimate/CI/sample/p-value fields, threshold, point/interval relation, and conclusion. This is an exact comparison of recorded canonical snapshots; no display rounding can conceal a changed p-value. Existing Phase 7 validation governs interval semantics, including valid percentile-bootstrap intervals outside the original estimate and partial interpretations with a missing effect component.
...
`reproduce(record, data=frame)` first checks a recorded fingerprint. A mismatch stops before execution by default. `allow_changed_data=True` permits a labelled changed-data rerun, never a same-data reproduction. Without a usable fingerprint, replay can compare numbers but returns `unavailable` for the same-data claim. It revalidates the specification, requires the same method to remain selected, then calls the existing Phase 6 engine only on explicit request. Valid Student t and one-way ANOVA adapter records cannot be silently routed through a different guided selector choice. The original result is never replaced. For schema version 2, replay still executes only the base analysis and warns that recorded Phase 11 scenarios were not automatically replayed.
