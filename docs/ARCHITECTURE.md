...
Future phases can add dataset-aware validation, method selection, execution, interpretation, provenance, and report assembly around these contracts. A GUI should serialize these records and render them without duplicating statistical rules. The existing automatic independent-group test selection remains a legacy analyzer behavior pending Phase 2 review. The new facade does not call it.

Compatibility policy: existing public imports, method signatures, analysis dictionary keys, numerical algorithms, and exports remain intact. Schema version 1 is the first configuration wire format; a future incompatible schema change must use a new version and explicitly document migration. The new records do not retrofit legacy result dictionaries.
