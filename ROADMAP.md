...
| 3. Automatic test selection | `hypothesis_tests(test_type="auto", estimand=...)` selects supported independent-group methods. The guided `run()` path checks the stated design and target, executes one ready method, and preserves the same result through interpretation, reporting, audit, and reproducibility metadata. | Add justified multi-group mean support and broader method coverage. |
